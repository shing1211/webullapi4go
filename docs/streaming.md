...
only carries pushes. Build a streaming client from a configured
`*client.Client`:
...
| `Category` | Security type: `CategoryUSStock`, `CategoryUSETF`, `CategoryHKStock`, `CategoryCNStock` |
| `SubTypes` | Data types: `SubTypeQuote`, `SubTypeSnapshot`, `SubTypeTick` |
| `Grab` | Request an immediate snapshot push on subscription |
| `Depth` | Level-2 order-book depth (optional; defaults to 10, US stocks up to 50) |
...
| `OnError` | `error` | Async errors (decode failures, unknown topics) |
