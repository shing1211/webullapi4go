# Webull OpenAPI — Master Reference (verbatim)

> Verbatim snapshot of every Webull-published **endpoint definition** (OpenAPI schema), split by area. No SDK-specific content. See [Master Guides](master-guides.md) for authentication, streaming protocol, trading rules and error codes.

| | |
|---|---|
| **Snapshot** | 2026-09-22 |
| **Sources** | [developer.webull.hk](https://developer.webull.hk/apis/llms.txt) and [developer.webull.com](https://developer.webull.com/apis/llms.txt) |
| **Scope** | All documented endpoints, HTTP and gRPC |
| **Refresh** | `python docgen.py master` |

[<- Webull API Reference](../webull-api.md)

## Areas

- [Authentication](reference/authentication.md) — 4 endpoints
- [Market Data — Stock](reference/market-data-stock.md) — 7 endpoints
- [Market Data — Option](reference/market-data-option.md) — 4 endpoints
- [Market Data — Crypto](reference/crypto.md) — 3 endpoints
- [Market Data — Futures](reference/market-data-futures.md) — 8 endpoints
- [Market Data — News](reference/market-data-news.md) — 1 endpoints
- [Market Data — Screener](reference/market-data-screener.md) — 6 endpoints
- [Market Data — Watchlist](reference/market-data-watchlist.md) — 8 endpoints
- [Fundamentals and Fund Data](reference/fundamentals.md) — 23 endpoints
- [Event Contracts](reference/event-contracts.md) — 19 endpoints
- [Trading API](reference/trading.md) — 13 endpoints
- [Broker API — HK](reference/broker-hk.md) — 36 endpoints
- [Broker API — FD (US)](reference/broker-fd-us.md) — 50 endpoints
- [Display Solution](reference/display-solution.md) — 20 endpoints
- [Streaming (MQTT)](reference/streaming.md) — 2 endpoints
- [Events (gRPC)](reference/events.md) — 3 endpoints
- [Connect API (OAuth)](reference/connect-api.md) — 2 endpoints

