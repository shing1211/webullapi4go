...
func newEventMetricReader(t *testing.T) (*sdkmetric.ManualReader, *sdkmetric.MeterProvider) {
	t.Helper()
...
func collectEventMetrics(t *testing.T, reader *sdkmetric.ManualReader) metricdata.ResourceMetrics {
	t.Helper()
...
func findEventMetric(t *testing.T, rm metricdata.ResourceMetrics, scope, name string) metricdata.Metrics {
	t.Helper()
...
func assertEventAttemptMetric(t *testing.T, metric metricdata.Metrics) {
	t.Helper()
	data, ok := metric.Data.(metricdata.Sum[int64])
	if !ok || len(data.DataPoints) != 1 || data.DataPoints[0].Value != 1 {
		t.Fatalf("event attempt data = %#v, want one value of 1", metric.Data)
	}
...
func assertEventDurationMetric(t *testing.T, metric metricdata.Metrics, status int64, outcome string) {
	t.Helper()
	data, ok := metric.Data.(metricdata.Histogram[float64])
	if !ok || len(data.DataPoints) != 1 || data.DataPoints[0].Count != 1 {
		t.Fatalf("event duration data = %#v, want one sample", metric.Data)
	}
	attrs := data.DataPoints[0].Attributes
	for key, want := range map[string]string{
		"rpc.system":  "grpc",
...
func TestEventMetricsRecordAttemptAndDurationForSuccess(t *testing.T) {
	reader, provider := newEventMetricReader(t)
	core := newCoreWithOptions(t, client.WithMeterProvider(provider))
	cl := newClientWithCore(t, core, &metadataServer{}, events.WithAutoReconnect(false))
	if err := cl.Run(context.Background()); err != nil {
		t.Fatalf("Run() error = %v", err)
	}

	rm := collectEventMetrics(t, reader)
	assertEventAttemptMetric(t, findEventMetric(t, rm, "webullapi4go/events", "event_stream_attempts"))
	assertEventDurationMetric(t, findEventMetric(t, rm, "webullapi4go/events", "event_stream_attempt_duration"), 0, "ok")
}

func TestEventMetricsRecordAttemptAndDurationForCancellation(t *testing.T) {
	reader, provider := newEventMetricReader(t)
	core := newCoreWithOptions(t, client.WithMeterProvider(provider))
	...
	rm := collectEventMetrics(t, reader)
	assertEventAttemptMetric(t, findEventMetric(t, rm, "webullapi4go/events", "event_stream_attempts"))
	assertEventDurationMetric(t, findEventMetric(t, rm, "webullapi4go/events", "event_stream_attempt_duration"), 1, "error")
}

func TestEventWithNilMeterProviderDoesNotPanic(t *testing.T) {
