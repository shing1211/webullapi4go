...
const (
	eventGRPCService            = "grpc.trade.event.EventService"
	eventGRPCMethod             = "Subscribe"
	eventGRPCPath               = "/grpc.trade.event.EventService/Subscribe"
	eventCorrelationMetadataKey = "x-correlation-id"
)
...
func newEventMetrics(cfg *observability.Config) *eventMetrics {
	...
	meter := cfg.Meter("webullapi4go/events")
	if meter == nil {
		return nil
	}
...
func (c *Client) startEventAttempt(ctx context.Context, attempt int) (context.Context, *eventAttempt) {
	...
	if state.cfg != nil {
		if tracer := state.cfg.Tracer("webullapi4go/events"); tracer != nil {
			ctx, state.span = tracer.Start(ctx, eventGRPCPath,
				trace.WithSpanKind(trace.SpanKindClient),
				trace.WithAttributes(
					attribute.String("rpc.system", "grpc"),
					attribute.String("rpc.service", eventGRPCService),
					attribute.String("rpc.method", eventGRPCMethod),
		...
		if log := state.cfg.Logger; log != nil {
			log.LogAttrs(state.ctx, slog.LevelInfo, "webull events stream start",
				slog.String("transport", "grpc"),
				slog.String("service", eventGRPCService),
				slog.String("method", eventGRPCMethod),
...
func (a *eventAttempt) finish(err error) {
	...
	if a.cfg != nil && a.cfg.Logger != nil {
		...
		a.cfg.Logger.LogAttrs(a.ctx, level, "webull events stream done", attrs...)
	}
	if a.span != nil {
		a.span.End()
	}
...
func (c *Client) addEventMetadata(ctx context.Context, md metadata.MD) metadata.MD {
	...
	if id := client.CorrelationIDFromContext(ctx); id != "" {
		md.Set(eventCorrelationMetadataKey, id)
	}
	return md
}
