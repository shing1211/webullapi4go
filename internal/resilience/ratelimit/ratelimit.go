// Copyright 2026 shing1211
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

// Package ratelimit implements a token-bucket rate limiter and a keyed
// variant for applying independent limits per endpoint.
//
// A [Limiter] is created with a refill rate (tokens per second) and a burst
// capacity. [Limiter.Allow] reports whether a token is available without
// blocking, while [Limiter.Wait] blocks until a token is available or ctx is
// done. A [Keyed] limiter lazily creates one [Limiter] per key so callers can
// enforce per-endpoint budgets from a single value.
package ratelimit

import (
	"context"
	"math"
	"sync"
	"time"

	"github.com/shing1211/webullapi4go/internal/resilience/clock"
)

// Limiter is a token-bucket rate limiter. It is safe for concurrent use.
type Limiter struct {
	mu    sync.Mutex
	rate  float64
	burst float64
	toks  float64
	last  time.Time
	clk   clock.Clock
}

// Option mutates the configuration of a [Limiter]. nil options are ignored.
type Option func(*config)

// config holds the resolved options for a [Limiter].
type config struct {
	clock clock.Clock
}

// WithClock overrides the time source. A nil clock is replaced by the
// real-time clock.
func WithClock(clk clock.Clock) Option {
	return func(c *config) { c.clock = clk }
}

// New returns a [Limiter] that refills at rate tokens per second and holds at
// most burst tokens. New panics when rate or burst is not positive.
func New(rate float64, burst int, opts ...Option) *Limiter {
	if rate <= 0 {
		panic("ratelimit: rate must be positive")
	}
	if burst <= 0 {
		panic("ratelimit: burst must be positive")
	}
	cfg := config{clock: clock.System()}
	for _, opt := range opts {
		if opt != nil {
			opt(&cfg)
		}
	}
	if cfg.clock == nil {
		cfg.clock = clock.System()
	}
	return &Limiter{
		rate:  rate,
		burst: float64(burst),
		toks:  float64(burst),
		last:  cfg.clock.Now(),
		clk:   cfg.clock,
	}
}

// refill adds the tokens accrued since the last update and returns the current
// token count. The caller must hold l.mu.
func (l *Limiter) refill(now time.Time) float64 {
	elapsed := now.Sub(l.last).Seconds()
	if elapsed > 0 {
		l.toks = math.Min(l.burst, l.toks+elapsed*l.rate)
		l.last = now
	}
	return l.toks
}

// Allow reports whether a token is available, consuming it when it is.
func (l *Limiter) Allow() bool {
	l.mu.Lock()
	defer l.mu.Unlock()
	if l.refill(l.clk.Now()) >= 1 {
		l.toks--
		return true
	}
	return false
}

// Tokens returns the currently available number of tokens.
func (l *Limiter) Tokens() float64 {
	l.mu.Lock()
	defer l.mu.Unlock()
	return l.refill(l.clk.Now())
}

// Wait blocks until a token is available and consumes it, or until ctx is
// done. It returns ctx.Err() when ctx is canceled or its deadline passes.
func (l *Limiter) Wait(ctx context.Context) error {
	for {
		l.mu.Lock()
		now := l.clk.Now()
		if l.refill(now) >= 1 {
			l.toks--
			l.mu.Unlock()
			return nil
		}
		need := (1 - l.toks) / l.rate
		l.mu.Unlock()

		delay := time.Duration(need * float64(time.Second))
		if delay < time.Millisecond {
			// Guard against busy-looping when a token is a fraction of a
			// millisecond away.
			delay = time.Millisecond
		}
		select {
		case <-ctx.Done():
			return ctx.Err()
		case <-l.clk.After(delay):
		}
	}
}

// Keyed applies an independent token-bucket limit per key. Limiters are
// created lazily by the factory supplied to [NewKeyed]. It is safe for
// concurrent use.
type Keyed struct {
	mu       sync.Mutex
	factory  func() *Limiter
	limiters map[string]*Limiter
}

// NewKeyed returns a [Keyed] limiter whose per-key limiters are produced by
// factory. factory must not be nil.
func NewKeyed(factory func() *Limiter) *Keyed {
	if factory == nil {
		panic("ratelimit: NewKeyed factory must not be nil")
	}
	return &Keyed{factory: factory, limiters: make(map[string]*Limiter)}
}

// For returns the limiter for key, creating it on first use.
func (k *Keyed) For(key string) *Limiter {
	k.mu.Lock()
	defer k.mu.Unlock()
	l, ok := k.limiters[key]
	if !ok {
		l = k.factory()
		k.limiters[key] = l
	}
	return l
}

// Set installs a specific limiter for key, overriding the factory for that
// key. It is useful for per-endpoint overrides.
func (k *Keyed) Set(key string, l *Limiter) {
	k.mu.Lock()
	defer k.mu.Unlock()
	k.limiters[key] = l
}

// Allow reports whether a token is available for key.
func (k *Keyed) Allow(key string) bool { return k.For(key).Allow() }

// Wait blocks until a token is available for key or ctx is done.
func (k *Keyed) Wait(ctx context.Context, key string) error { return k.For(key).Wait(ctx) }
